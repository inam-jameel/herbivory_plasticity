...
mod_reproduction1<- glmer(Flowered~treatment+elev_km+garden+ (1|block)+(1|Genotype), data = surv, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e7)), family=binomial)
...
mod_repro<- glmer(Flowered~treatment* elev_dist_km+I(elev_dist_km^2)*treatment +(1|block)+(1|Genotype), data = data, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e7)), family=binomial) 
...
data_LAR<-data %>% dplyr::select(treatment, S_elev, elev_km,garden,block_garden,garden_treat,Genotype,LAR_prop)
...
#So, we might be able to remove the tau component from the model, and run a zero-inflated model:
  ...
  gamlss.modelB<- gamlss (formula=LAR_prop~treatment+S_elev+garden+ random(block_garden)+random(Genotype), 
                          sigma.formula=LAR_prop~treatment+S_elev+garden+ random(block_garden)+random(Genotype), 
                          nu.formula=LAR_prop~treatment+S_elev+garden+ random(block_garden)+random(Genotype), family=BEINF, data= data_LAR)
...
visreg(gamlss.modelB, "S_elev", type="conditional", #scale = "response", 
...
modC<- lmer (emmean~elev_km+treatment+garden+(1|Genotype),control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e7)), data=LSmeans_LAR)


Anova(modC)

visreg(modC,overlay=F,"elev_km", by ="garden")
