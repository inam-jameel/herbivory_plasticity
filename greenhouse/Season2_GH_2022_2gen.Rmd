...
knitr::opts_knit$set(root.dir = "~/Users/inam/Library/CloudStorage/OneDrive-UniversityofGeorgia/Inam_experiments/Herbivory_data/greenhouse/")
```
...
setwd("~/OneDrive - University of Georgia/Inam_experiments/Herbivory_data/greenhouse/")
...
vioplot(S2_SLA ~ treatment, data= gh2, plotCentre = "point",  pchMed = 23,  horizontal= FALSE,colMed = "black",colMed2 = c("#6699cc","#882255"), col=c("#6699cc","#882255"), ylab="Relative growth rate", xlab="Minimum temperature (°C)")+stripchart(S2_SLA ~ treatment, data= gh2,  method = "jitter", col = alpha("black", 0.2), pch=16 
...
p_SLA <- subset(gh2, mat_treat == "Parent")

parents_SLA<- lmer(S2_SLA ~ treatment * elev +S_init_size+(1|block) + (1|genotype),control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e7)), data = p_SLA)
Anova(parents_SLA)
plot (parents_SLA)
...
visreg(parents_SLA, overlay = TRUE, "elev", by="treatment", type="conditional", scale = "response", 
  ...
  SLAgga<-ggplot(p_SLA, aes(x = treatment, y = S2_SLA, fill = treatment,)) +
  geom_boxplot(outlier.shape = NA) +xlab("Herbivore treatment")+ scale_y_continuous("SLA CM3/g") +
  geom_point(pch = 21, position = position_jitterdodge())
SLAgga + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(), 
                           panel.grid.minor=element_blank(),legend.position = "top")+ scale_x_discrete(labels=c("No herbivory", "Herbivorized")) +  scale_fill_manual(values = c( "lightgreen","lightyellow"), #name = "Watering treatment", labels = c("Ample water","Water-restricted")
                                                                                                                                                                      )
...
modB<- lmer(S2_SLA ~ genotype*mat_treat*treatment+S_init_size +(1|block),control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e7)), data = gh2)
...
write.csv(LSmeans_SLA,file="LSmeans_SLA_S2.csv")

```

```{r}
#read in emmeans
LSmeans_SLA <- read.csv("LSmeans_SLA_S2.csv", stringsAsFactors=TRUE)
...
RC_SLA_means<- lm(emmean ~ elevation+mat_treat+treatment, data = LSmeans_SLA)
Anova(RC_SLA_means)
plot(RC_SLA_means)
...
### Trichomes individual from season 1

```{r}
hist(gh2$trichomes)
  
SLAgga<-ggplot(gh2, aes(x = mat_treat, y = trichomes, fill = treatment)) +
  geom_boxplot(outlier.shape = NA) +xlab("Maternal treatment")+ scale_y_continuous("Trichomes") +
  geom_point(pch = 21, position = position_jitterdodge())
SLAgga + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(), 
                           panel.grid.minor=element_blank(),legend.position = "top")+ scale_x_discrete(labels=c("No Herbivores", "Herbivorized", "Parent Control")) +  scale_fill_manual(values = c( "gold","purple"), name = "Herbivore treatment", labels = c("No Herbivores","Herbivorized"))
...
gh_sla= ggplot(gh2, aes(x=elev,y= trichomes, group= treatment, 
                               colour= treatment))+geom_point(size=5) + scale_y_continuous("Trichomes")+ scale_x_continuous("Source Elevation")  
gh_sla + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(),
                           panel.grid.minor=element_blank(),legend.position = "top")+geom_smooth(method="lm",size=1.6, formula=y~x)+facet_wrap(~ mat_treat, scales="free_x") +scale_colour_manual(values = c( "gold","purple","grey"), name = "Herbivore treatment", labels = c("No Herbivores","Herbivorized"))

gh2 $mat_herb<-interaction(gh2 $mat_treat, gh2 $treatment,sep = "_")

RC_Tri<- glmer(trichomes ~ mat_herb*S_elev +(1|block)+(1|genotype),family = Gamma(link=log), data = gh2) 
...
trichomes <-predictorEffect("S_elev",  partial.residuals=TRUE, mod = RC_Tri)

plot(trichomes, lwd=2,xlab="Source Elevation (KM)", ylab="Fruit", pch=19, type="response",lines=list(multiline=FALSE, #lty=2:1,
                                                                                                 col="black"), partial.residuals=list(smooth=FALSE, pch=19, col="black"), ylim=c(0,200))


visreg(RC_Tri, "S_elev", by="mat_herb",overlay = FALSE,type="conditional", scale = "response", 
...
#read in emmeans
#LSmeans_tri <- read.csv("LSmeans_tri_S1.csv", stringsAsFactors=TRUE)
...
LSmeans_tri $S_elev<-scale(LSmeans_tri$elevation,center=TRUE, scale=TRUE)



RC_tri_means<- glm(emmean ~ mat_treat+S_elev+treatment, data = LSmeans_tri) 
...
SLA <- lm(emmean ~ mat_treat*treatment, data = gh2)
Anova (SLA)
...
gh_pf + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(),
                           panel.grid.minor=element_blank(),legend.position = "top")+geom_smooth(method="glm",size=1.6, formula=y~x)+facet_wrap(~ mat_treat, scales="free_x") +scale_colour_manual(values = c( "gold","purple","grey"), name = "Herbivore treatment", labels = c("No Herbivores","Herbivorized"))

gh_pf= ggplot(gh2, aes(x= elev,y= S2_flowered, group= treatment, 
                               colour= treatment))+geom_point(size=5) + scale_y_continuous("Probability of flowering")+ scale_x_continuous("Source Elevation")  
gh_pf + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(),
                           panel.grid.minor=element_blank(),legend.position = "top")+geom_smooth(method="glm",size=1.6, formula=y~x)+facet_wrap(~ mat_treat, scales="free_x") +scale_colour_manual(values = c( "gold","purple","grey"), name = "Herbivore treatment", labels = c("No Herbivores","Herbivorized"))

gh_pf= ggplot(subset(gh2, mat_treat!="Parent"), aes(x= elev,y= S2_flowered, group= treatment, 
                               colour= treatment))+geom_point(size=5) + scale_y_continuous("Probability of flowering")+ scale_x_continuous("Source Elevation")  
gh_pf + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(),
                           panel.grid.minor=element_blank(),legend.position = "top")+geom_smooth(method="glm",size=1.6, formula=y~x)+facet_wrap(~ mat_treat, scales="free_x") +scale_colour_manual(values = c( "gold","purple","grey"), name = "Herbivore treatment", labels = c("No Herbivores","Herbivorized"))

SLAgga<-ggplot(gh2, aes(x = mat_treat, y = Overall_flowered, fill = treatment)) +
  geom_boxplot(outlier.shape = NA) +xlab("Maternal treatment")+ scale_y_continuous("SLA CM3/g") +
  geom_point(pch = 21, position = position_jitterdodge())
SLAgga + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(), 
...
mod_flower<- glmer(Overall_flowered~ elev*treatment+mat_treat*elev+ S_init_size + (1|block)+(1|genotype), data = gh2, control=glmerControl(optimizer="optimx", tolPwrss=1e-3, optCtrl=list(method="nlminb")), family=binomial)
...
mod_pr2<- glmer(Overall_flowered~ treatment*elev*mat_treat+ S_init_size+(1|block)+(1|genotype), data =subset(gh2, mat_treat!="Parent"), control=glmerControl(optimizer="optimx", tolPwrss=1e-3,optCtrl=list(method="nlminb")), family=binomial)

Anova(mod_pr2)
flowerP2 <-predictorEffect("elev",  partial.residuals=TRUE, mod_pr2)

plot(flowerP2, lwd=2,xlab="Source Elevation (KM)", ylab="Probability of reproduction", pch=19, type="response",lines=list(multiline=TRUE, lty=2:1,
                                                                              col="black"), partial.residuals=list(smooth=FALSE, pch=19, col="black"), ylim=c(0,1))

mod_pr3<- glmer(S2_flowered~ treatment*elev*mat_treat+ S_init_size+(1|block)+(1|genotype), data =subset(gh2, mat_treat!="Parent"), control=glmerControl(optimizer="optimx", tolPwrss=1e-3,optCtrl=list(method="nlminb")), family=binomial)

Anova(mod_pr3)
flowerP3 <-predictorEffect("elev",  partial.residuals=TRUE, mod_pr3)
...
Anova(mod_pr4)
flowerP4 <-predictorEffect("elev",  partial.residuals=TRUE, mod_pr4)

plot(flowerP4, lwd=2,xlab="Source Elevation (KM)", ylab="Probability of reproduction", pch=19, type="response",lines=list(multiline=TRUE, lty=2:1,
                                                                              col="black"), partial.residuals=list(smooth=FALSE, pch=19, col="black"), ylim=c(0,1))
...
```{r}
gh_pf= ggplot(gh2, aes(x= elev,y= round(Overall_Mature_length_siliques), group= treatment, 
...
SLAgga<-ggplot(gh2, aes(x = mat_treat, y = round(Overall_Mature_length_siliques), fill = treatment)) +
  geom_boxplot(outlier.shape = NA) +xlab("Maternal treatment")+ scale_y_continuous("Fecundity(summed silique length") +
  geom_point(pch = 21, position = position_jitterdodge())
SLAgga + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(), 
                           panel.grid.minor=element_blank(),legend.position = "top")+ scale_x_discrete(labels=c("No Herbivores", "Herbivorized", "Parent Control")) +  scale_fill_manual(values = c( "gold","purple"), name = "Herbivore treatment", labels = c("No Herbivores","Herbivorized"))
...
plot(gh2$Overall_Mature_length_siliques,gh2$S1_LAR3)

herb <- filter(gh2,treatment!="Control")

gh_pf= ggplot(herb, aes(x= (LAR_prop2),y= round(S2_Mature_length_siliques), group= treatment, 
                               colour= treatment))+geom_point(size=5) + scale_y_continuous("Fecundity(summed silique length)")+ scale_x_continuous("Source Elevation")  
gh_pf + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(),
                           panel.grid.minor=element_blank(),legend.position = "top")+geom_smooth(method="glm",size=1.6, formula=y~x)+facet_wrap(~ mat_treat, scales="free_x") +scale_colour_manual(values = c("purple"), name = "Herbivore treatment", labels = c("Herbivorized"))


gh_pf= ggplot(gh2, aes(x= elev,y= S1_LAR3, group= treatment, 
                               colour= treatment))+geom_point(size=5) + scale_y_continuous("Fecundity(summed silique length)")+ scale_x_continuous("Source Elevation")  
gh_pf + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(),
                           panel.grid.minor=element_blank(),legend.position = "top")+geom_smooth(method="glm",size=1.6, formula=y~x)+facet_wrap(~ mat_treat, scales="free_x") +scale_colour_manual(values = c( "gold","purple","grey"), name = "Herbivore treatment", labels = c("No Herbivores","Herbivorized"))
...
hurdle <- glmmTMB (round(Overall_Mature_length_siliques) ~ treatment+mat_treat+elev+S_init_size  + (1| block)+(1|genotype), zi=~ treatment+mat_treat+elev+S_init_size  + (1| block)+(1|genotype),data = gh2 ,family=truncated_nbinom2)
...
fruit <-predictorEffect("treatment",  partial.residuals=TRUE, hurdle,component="zi")

plot(fruit, lwd=2,xlab="Source Elevation (KM)", ylab="Fruit", type="response",lines=list(multiline=FALSE, #lty=2:1,
                                                                                                 col="black"), partial.residuals=list(smooth=FALSE, col="black"), ylim=c(0,1500))
...
LAR_3 <- drop_na(LAR_data,S1_LAR3) #this removes the rows without LAR values
LAR_3 <- dplyr::select(LAR_3, S1_LAR3, elev, genotype, treatment, mat_treat, exp_ID, S_init_size, block)

#response variable cant have zeros, so I added 0.0001 to every value

LAR_3$S1_LAR3 <- LAR_3$S1_LAR3/100
gamlss_mod1<- gamlss (formula=S1_LAR3~elev*mat_treat+ S_init_size+ random(block)+random(genotype), family=BE, data= LAR_3)
summary(gamlss_mod1)
plot(gamlss_mod1)
...
h_lar= ggplot(LAR_3, aes(x= elev,y= S1_LAR3, group= mat_treat, 
...
h_lar + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
 ...
 modA<- lmer(Overall_Mature_length_siliques~ S1_LAR3+ S_init_size+ (1|block)+(1|genotype),data= herb)
 Anova(modA,type="III")
 ##You can see here that the residuals aren't great
 plot(modA)
 ...
 visreg(modA, 'S1_LAR3', type="conditional", 
       #scale = "response", 
       xlab="Elevation (m)", ylab="Percentage leaf area herbivorized", partial=TRUE, cex.lab = 1.5,cex.axis = 1.5,
       fill=list(col="blue"),
       line=list(col=grey(c(0,0.8))),
 ...
 h_lar= ggplot(herb, aes(x=elevation,y= S1_Mature_length_siliques, group= mat_treat, 
                               colour= mat_treat))+geom_point(size=5) + scale_y_continuous("Fecundity (Silique Length)",limits =c(0, 1000))+ scale_x_continuous("Leaf Area Herbivorized")  
h_lar + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(),
                           panel.grid.minor=element_blank(),legend.position = "top")+geom_smooth(method="lm",size=1.6, formula=y~x)+facet_wrap(~ mat_treat, scales="free_x") +scale_colour_manual(values = c( "purple", "purple", "purple"))#, name = "Watering treatment", labels = c("Ample water","Water-restricted"))

hurdle3 <- glmmTMB (round(Overall_Mature_length_siliques) ~ S1_LAR3+S_init_size  + (1| block)+(1|genotype), zi=~ S1_LAR3+S_init_size  + (1| block)+(1|genotype),data = herb ,family=truncated_nbinom2)
...
fruit2 <-predictorEffect("S1_LAR3",  partial.residuals=TRUE, hurdle3,component="zi")
...
#lmer model
  modA<- lmer(Overall_flowered~ S1_LAR3+mat_treat+ S_init_size+ (1|block)+(1|genotype),data= herb)
 Anova(modA,type="III")
 ##You can see here that the residuals aren't great
 plot(modA)
 ...
 h_lar= ggplot(herb, aes(x=Overall_flowered,y= (S1_LAR3/100), group= mat_treat, 
                               colour= mat_treat))+geom_point(size=5) + scale_y_continuous("Leaf Area Herbivorized",limits =c(0, 1))+ scale_x_continuous("Source Elevation")  
h_lar + theme_bw() + theme(text = element_text(size=20),axis.line.x = element_line(colour = "black"), 
                           axis.line.y = element_line(colour = "black"), panel.border = element_blank(), panel.grid.major =element_blank(),
                           panel.grid.minor=element_blank(),legend.position = "top")+geom_smooth(method="lm",size=1.6, formula=y~x)+facet_wrap(~ mat_treat, scales="free_x") +scale_colour_manual(values = c( "purple", "purple", "purple"))#, name = "Watering treatment", labels = c("Ample water","Water-restricted"))
