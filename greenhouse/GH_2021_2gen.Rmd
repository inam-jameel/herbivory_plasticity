...
parents_SLA<- lmer(SLA100 ~ treatment +(1|block) + (1|genotype),control = lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e7)), data = p_SLA)
Anova(parents_SLA)
plot (parents_SLA)

visreg(parents_SLA, overlay = TRUE, "genotype", by="treatment", type="conditional", scale = "response", 
       xlab="Elevation (m)", ylab="Specific leaf area", partial=TRUE, cex.lab = 1.5,cex.axis = 1.5,
       fill=list(col=grey(c(0.7,0.9))),
       line=list(col=grey(c(0,0.5))),
       points=list(cex=1.5,col=grey(c(0.2,0.8))))  
...
parents1 <- p_SLA %>% 
  group_by(treatment) %>%
  get_summary_stats(SLA100, type="mean_se")
parents1
