...
grasshopper$duration<-(grasshopper $Date_silique - grasshopper $Ordinal_Date_flowering)
...
traitdat $SDUR<-scale(traitdat $duration,center=TRUE,scale=TRUE)
...
summary(repro_model_full)

coef_repSLA <- emtrends(repro_model_full, specs = c("Water"), var = "sSLA")
repSLA<- as.data.frame(summary(coef_repSLA))[c('Water', 'sSLA.trend', 'SE')]
repSLA_table <- repSLA%>% mutate(
...
coef_repSUC <- emtrends(repro_model_full, specs = c("Herbivore"), var = "sSUC")
repSUC<- as.data.frame(summary(coef_repSUC))[c('Herbivore', 'sSUC.trend', 'SE')]
repSUC_table <- repSUC%>% mutate(
...
##I have to precode the quadratic effects here.
traitdat$SLA2<-traitdat$sSLA * traitdat$sSLA
traitdat$SUC2<-traitdat$sSUC * traitdat$sSUC
repromod2 <-glmmTMB(Reproduced~S_initdiam+Water*Herbivore*year+
                                   Water*Herbivore*sLAR+
                                   Water*Herbivore*sSUC+ SUC2 +
                                   Water*Herbivore*sSLA+ SLA2 
                                 +(1|PlantID)+(1|Cage_Block)+(1|Genotype),data=traitdat,family=binomial(link="logit"),control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
Anova(repromod2,type="III") 
summary(repromod2)


coef_SLA2 <- emtrends(repromod2, specs = c("SLA2"), var = "SLA2")
repSLA2<- as.data.frame(summary(coef_SLA2))[c('SLA2.trend', 'SE')]
repSLA2_table <- repSLA2%>% mutate(
  oddsratio = exp(SLA2.trend),
  Lower95 = oddsratio * exp(-1.96*SE),
  Upper95 = oddsratio * exp(1.96*SE))
...
coef_SUC2 <- emtrends(repromod2, specs = c("SUC2"), var = "SUC2")
repSUC2<- as.data.frame(summary(coef_SUC2))[c('SUC2.trend', 'SE')]
repSUC2_table <- repSUC2%>% mutate(
  oddsratio = exp(SUC2.trend),
  Lower95 = oddsratio * exp(-1.96*SE),
  Upper95 = oddsratio * exp(1.96*SE))

repSUC2_table

##Optimum SLA in Water restricted treatment
optSLA_restrict<-(-repSLA[1,2]/(2* repSLA2[1,1]))
##lower
lowerSLA_restrict<-(-(repSLA[1,2]-1.96*repSLA[1,3])/(2* (repSLA2[1,1]-1.96*repSLA2[1,2])))
##Upper
upperSLA_restrict<--(repSLA[1,2]+1.96*repSLA[1,3])/(2* (repSLA2[1,1]+1.96*repSLA2[1,2]))

##To convert from traits in standardized units to raw units
optSLA_restrict*sd(traitdat $SLA,na.rm = TRUE)+mean(traitdat $SLA,na.rm = TRUE)
lowerSLA_restrict*sd(traitdat $SLA,na.rm = TRUE)+mean(traitdat $SLA,na.rm = TRUE)
upperSLA_restrict*sd(traitdat $SLA,na.rm = TRUE)+mean(traitdat $SLA,na.rm = TRUE)

##Optimum SLA in Supplemental water treatment
optSLA_supp<--repSLA[2,2]/(2* repSLA2[1,1])
##lower
lowerSLA_supp <--(repSLA[2,2]-1.96*repSLA[2,3])/(2* (repSLA2[1,1]-1.96*repSLA2[1,2]))
##Upper
upperSLA_supp <--(repSLA[2,2]+1.96*repSLA[2,3])/(2* (repSLA2[1,1]+1.96*repSLA2[1,2]))

##To convert from traits in standardized units to raw units
optSLA_supp*sd(traitdat $SLA,na.rm = TRUE)+mean(traitdat $SLA,na.rm = TRUE)
lowerSLA_supp*sd(traitdat $SLA,na.rm = TRUE)+mean(traitdat $SLA,na.rm = TRUE)
upperSLA_supp*sd(traitdat $SLA,na.rm = TRUE)+mean(traitdat $SLA,na.rm = TRUE)



##Optimum SUC in Grasshopper addition treatment
optSUC_add<--repSUC[2,2]/(2* repSUC2[1,1])
##lower
lowerSUC_add<--(repSUC[2,2]-1.96*repSUC[2,3])/(2* (repSUC2[1,1]-1.96*repSUC2[1,2]))
##Upper
upperSUC_add<--(repSUC[2,2]+1.96*repSUC[2,3])/(2* (repSUC2[1,1]+1.96*repSUC2[1,2]))

##To convert from traits in standardized units to raw units (multiplied by 1000 to get to mg/cm^2)
1000*(optSUC_add*sd(traitdat $succulence,na.rm = TRUE)+mean(traitdat $succulence,na.rm = TRUE))
1000*(lowerSUC_add*sd(traitdat $succulence,na.rm = TRUE)+mean(traitdat $succulence,na.rm = TRUE))
1000*(upperSUC_add*sd(traitdat $succulence,na.rm = TRUE)+mean(traitdat $succulence,na.rm = TRUE))

##Optimum SUC in Grasshopper removal treatment
optSUC_removal<--repSUC[1,2]/(2* repSUC2[1,1])
##lower
lowerSUC_removal<--(repSUC[1,2]-1.96*repSUC[1,3])/(2* (repSUC2[1,1]-1.96*repSUC2[1,2]))
##Upper
upperSUC_removal<--(repSUC[1,2]+1.96*repSUC[1,3])/(2* (repSUC2[1,1]+1.96*repSUC2[1,2]))

##To convert from traits in standardized units to raw units (multiplied by 1000 to get to mg/cm^2)
1000*(optSUC_removal*sd(traitdat $succulence,na.rm = TRUE)+mean(traitdat $succulence,na.rm = TRUE))
1000*(lowerSUC_removal*sd(traitdat $succulence,na.rm = TRUE)+mean(traitdat $succulence,na.rm = TRUE))
1000*(upperSUC_removal*sd(traitdat $succulence,na.rm = TRUE)+mean(traitdat $succulence,na.rm = TRUE))
...
##For plotting
repro_mod <-glmmTMB(Reproduced~S_initdiam+Water*Herbivore*year+
                                   Water*Herbivore*sLAR+
                                   Herbivore*sSUC+I(sSUC^2)+
                                   Water*sSLA+I(sSLA^2) 
                                 +(1|PlantID)+(1|Cage_Block)+(1|Genotype),data=traitdat,family=binomial(link="logit"),control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
Anova(repro_mod,type="III") 
summary(repro_mod)

library(ggeffects)
cols=c("#D55E00", "blue")
full_sSUC <- ggpredict(repro_mod, terms = c("sSUC[all]", "Herbivore"), type = "re", interval="confidence")
...
visreg(repro_mod,"sSUC", by="Herbivore", overlay=TRUE, scale = "response", xlab="Succulence (standardized)", ylab="Probability of reproduction", partial=FALSE, type="conditional")


full_sla <- ggpredict(repro_mod, terms = c("sSLA[all]", "Water"), type = "re", interval="confidence")
...
##To caculate the emtrends for quadratic effect, we have to precode them
...
summary(fecund_modeltraits_quad)
emtrends(fecund_modeltraits_quad, specs = c("Water","Herbivore"), var = "sduration2")


##Then convert optimal values to raw scale
-0.6576063*sd(traitdatRepro $flowering_duration,na.rm = TRUE)+mean(traitdatRepro $flowering_duration,na.rm = TRUE)
-0.7275795*sd(traitdatRepro $flowering_duration,na.rm = TRUE)+mean(traitdatRepro $flowering_duration,na.rm = TRUE)
-0.4943947*sd(traitdatRepro $flowering_duration,na.rm = TRUE)+mean(traitdatRepro $flowering_duration,na.rm = TRUE)

0.98729006*sd(traitdatRepro $flowering_duration,na.rm = TRUE)+mean(traitdatRepro $flowering_duration,na.rm = TRUE)
0.32742952*sd(traitdatRepro $flowering_duration,na.rm = TRUE)+mean(traitdatRepro $flowering_duration,na.rm = TRUE)
3.64601406*sd(traitdatRepro $flowering_duration,na.rm = TRUE)+mean(traitdatRepro $flowering_duration,na.rm = TRUE)

0.84846013*sd(traitdatRepro $flowering_duration,na.rm = TRUE)+mean(traitdatRepro $flowering_duration,na.rm = TRUE)
0.40391701*sd(traitdatRepro $flowering_duration,na.rm = TRUE)+mean(traitdatRepro $flowering_duration,na.rm = TRUE)
1.97068591*sd(traitdatRepro $flowering_duration,na.rm = TRUE)+mean(traitdatRepro $flowering_duration,na.rm = TRUE)
...
emtrends(fecund_modeltraits, specs = c("Water","Herbivore"), var = "sSLA")
emtrends(fecund_modeltraits_quad, specs = c("Water","Herbivore"), var = "sSLA2")

##Then convert optimal values to raw scale
-0.1134342*sd(traitdatRepro $SLA,na.rm = TRUE)+mean(traitdatRepro $SLA,na.rm = TRUE)
-0.4022903*sd(traitdatRepro $SLA,na.rm = TRUE)+mean(traitdatRepro $SLA,na.rm = TRUE)
-2.1411204*sd(traitdatRepro $SLA,na.rm = TRUE)+mean(traitdatRepro $SLA,na.rm = TRUE)

-0.8532991*sd(traitdatRepro $SLA,na.rm = TRUE)+mean(traitdatRepro $SLA,na.rm = TRUE)
-0.7148272*sd(traitdatRepro $SLA,na.rm = TRUE)+mean(traitdatRepro $SLA,na.rm = TRUE)
-2.2003771*sd(traitdatRepro $SLA,na.rm = TRUE)+mean(traitdatRepro $SLA,na.rm = TRUE)

1.27365269*sd(traitdatRepro $SLA,na.rm = TRUE)+mean(traitdatRepro $SLA,na.rm = TRUE)
-0.2209287*sd(traitdatRepro $SLA,na.rm = TRUE)+mean(traitdatRepro $SLA,na.rm = TRUE)
-3.4863415*sd(traitdatRepro $SLA,na.rm = TRUE)+mean(traitdatRepro $SLA,na.rm = TRUE)
...
plot(standard_sSUC, show_data=TRUE, show_title =FALSE, show_legend=TRUE, colors = cols, facet=TRUE)+theme(text = element_text(size=10),axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), panel.border = element_blank(),panel.grid.major =element_blank(), panel.grid.minor=element_blank())+theme(legend.position="right")+scale_x_continuous("Succulence")+ scale_y_continuous("Fecundity")
 emtrends(fecund_modeltraits, specs = c("Water","Herbivore"), var = "sSUC")
 emtrends(fecund_modeltraits, specs = c("Herbivore"), var = "sSUC")
 #Conversion to raw scale
1000*(-2*sd(traitdatRepro $succulence,na.rm = TRUE)+mean(traitdatRepro $succulence,na.rm = TRUE))
1000*(-1*sd(traitdatRepro $succulence,na.rm = TRUE)+mean(traitdatRepro $succulence,na.rm = TRUE))
1000*(0*sd(traitdatRepro $succulence,na.rm = TRUE)+mean(traitdatRepro $succulence,na.rm = TRUE))
1000*(1*sd(traitdatRepro $succulence,na.rm = TRUE)+mean(traitdatRepro $succulence,na.rm = TRUE))
1000*(2*sd(traitdatRepro $succulence,na.rm = TRUE)+mean(traitdatRepro $succulence,na.rm = TRUE))
1000*(3*sd(traitdatRepro $succulence,na.rm = TRUE)+mean(traitdatRepro $succulence,na.rm = TRUE))
...
standard_height <- ggpredict(fecund_modeltraits, terms = c("s_max_height[all]"), type = "re", interval="confidence")
plot(standard_height, show_data=TRUE, show_title =FALSE, show_legend=TRUE, colors = cols, facet=TRUE)+theme(text = element_text(size=10),axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), panel.border = element_blank(),panel.grid.major =element_blank(), panel.grid.minor=element_blank())+theme(legend.position="right")+scale_x_continuous("Height")+ scale_y_continuous("Fecundity")+ylim(0,2000)
emtrends(fecund_modeltraits, specs = c("Water","Herbivore"), var = "s_max_height")
emtrends(fecund_modeltraits, specs = c("s_max_height"), var = "s_max_height")
 #Conversion to raw scale
...
#*******************************************************************************
####  Fecundity using ordinal day for flowering time  #####
#*******************************************************************************
traitdatRepro $sFP<-scale(traitdatRepro $Ordinal_Date_flowering,center=TRUE,scale=TRUE)
traitdatRepro $SDUR<-scale(traitdatRepro $duration,center=TRUE,scale=TRUE)


fecund_modeltraits_ordinal <-glmmTMB(Mature_length_siliques~ S_initdiam+Water*Herbivore*year +
                               Water*Herbivore*sFP+
                               Water*Herbivore* s_max_height +
                               Water*Herbivore* SDUR +Water*I(SDUR^2) +Herbivore*I(SDUR^2) +
                               Water*Herbivore* sSLA+ Water*Herbivore* I(sSLA^2)+
                               Water*Herbivore* sSUC+
                               Water*Herbivore* sLAR+ (1|Cage_Block)+(1|Genotype),data=traitdatRepro,family=Gamma(link="log"))
Anova(fecund_modeltraits_ordinal,type="III") 

simulationOutput <- simulateResiduals(fittedModel= fecund_modeltraits_ordinal, plot = T, re.form = NULL,allow.new.levels =TRUE)

#Context dependent selection
standard_FP <- ggpredict(fecund_modeltraits_ordinal, terms = c("sFP[all]", "Water","Herbivore"), type = "re", interval="confidence")
plot(standard_FP, show_data=TRUE, show_title =FALSE, show_legend=TRUE, colors = cols, facet=TRUE)+theme(text = element_text(size=10),axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), panel.border = element_blank(),panel.grid.major =element_blank(), panel.grid.minor=element_blank())+theme(legend.position="right")+scale_x_continuous("Flowering time ordinal")+ scale_y_continuous("Fecundity")

#Context dependent selection
standard_duration_ordinal<- ggpredict(fecund_modeltraits_ordinal, terms = c("SDUR[all]", "Water","Herbivore"), type = "re", interval="confidence")
plot(standard_duration_ordinal, show_data=TRUE, show_title =FALSE, show_legend=TRUE, colors = cols, facet=TRUE)+theme(text = element_text(size=10),axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), panel.border = element_blank(),panel.grid.major =element_blank(), panel.grid.minor=element_blank())+theme(legend.position="right")+scale_x_continuous("Flowering duration ordinal")+ scale_y_continuous("Fecundity")+ylim(0,2000)

#Context dependent selection
standard_sSLA_ordinal <- ggpredict(fecund_modeltraits_ordinal, terms = c("sSLA[all]", "Water","Herbivore"), type = "re", interval="confidence")
plot(standard_sSLA_ordinal, show_data=TRUE, show_title =FALSE, show_legend=TRUE, colors = cols, facet=TRUE)+theme(text = element_text(size=10),axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), panel.border = element_blank(),panel.grid.major =element_blank(), panel.grid.minor=element_blank())+theme(legend.position="right")+scale_x_continuous("Specific leaf area (ordinal model)")+ scale_y_continuous("Fecundity")

#Context dependent selection

standard_sSUC_ordinal <- ggpredict(fecund_modeltraits_ordinal, terms = c("sSUC[all]", "Water","Herbivore"), type = "re", interval="confidence")
plot(standard_sSUC_ordinal, show_data=TRUE, show_title =FALSE, show_legend=TRUE, colors = cols, facet=TRUE)+theme(text = element_text(size=10),axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), panel.border = element_blank(),panel.grid.major =element_blank(), panel.grid.minor=element_blank())+theme(legend.position="right")+scale_x_continuous("Succulence (ordinal model)")+ scale_y_continuous("Fecundity")

##No selection
standard_sLAR_ordinal <- ggpredict(fecund_modeltraits_ordinal, terms = c("sLAR[all]", "Water","Herbivore"), type = "re", interval="confidence")
plot(standard_sLAR_ordinal, show_data=TRUE, show_title =FALSE, show_legend=TRUE, colors = cols, facet=TRUE)+theme(text = element_text(size=10),axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), panel.border = element_blank(),panel.grid.major =element_blank(), panel.grid.minor=element_blank())+theme(legend.position="right")+scale_x_continuous("LAR (ordinal model)")+ scale_y_continuous("Fecundity")

standard_maxheight_ordinal <- ggpredict(fecund_modeltraits_ordinal, terms = c("s_max_height[all]", "Water"), type = "re", interval="confidence")
plot(standard_maxheight_ordinal, show_data=TRUE, show_title =FALSE, show_legend=TRUE, colors = cols, facet=TRUE)+theme(text = element_text(size=10),axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), panel.border = element_blank(),panel.grid.major =element_blank(), panel.grid.minor=element_blank())+theme(legend.position="right")+scale_x_continuous("Maximum height (ordinal model)")+ scale_y_continuous("Fecundity")+ylim(0,2000)

standard_maxheight_adjusted <- ggpredict(fecund_modeltraits, terms = c("s_max_height[all]", "Water"), type = "re", interval="confidence")
plot(standard_maxheight_adjusted, show_data=TRUE, show_title =FALSE, show_legend=TRUE, colors = cols, facet=TRUE)+theme(text = element_text(size=10),axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), panel.border = element_blank(),panel.grid.major =element_blank(), panel.grid.minor=element_blank())+theme(legend.position="right")+scale_x_continuous("Maximum height")+ scale_y_continuous("Fecundity")+ylim(0,2000)
