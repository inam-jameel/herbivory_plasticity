...
reproductive<-subset(GH, State=="Reproductive")
...
Empty_input<-read.csv("grasshopper_empty_2023.csv", header=T)
...
for (i in 1:length(unique(GH $PlantID))) {
  
  #        ##Subset for each plant
  target_ID<-unique(GH $PlantID)[i]
  sub<-subset(GH, PlantID==target_ID)
...
#overwinter_survival - Census 2 was the first full census, alive=1 dead=0
ifelse((sub$Status[2]=="Alive"), Empty$Overwinter_survival_2023[target_row]<-1, Empty$Overwinter_survival_2023[target_row]<-0)


#season_survival - Census 10 was the last full census, alive=1 dead=0
ifelse((sub$Status[10]=="Alive"), Empty$Season_survival_2023[target_row]<-1, Empty$Season_survival_2023[target_row]<-0)
...
Empty$Exclude_2023[target_row]<-sub$Include[10] 
...
##I measured leaf damage at censuses 5, and 7

Empty$num_total5_2023[target_row]<-sub$num_total_L[5] 
Empty$num_damaged5_2023[target_row]<-sub$number_damaged[5] 
Empty$total_damage5_2023[target_row]<-sub$total_damage[5]
Empty$avg_damage5_2023[target_row]<-sub$average_damage[5]
Empty$LAR_5_2023[target_row]<-sub$LAR[5] 

Empty$num_total7_2023[target_row]<-sub$num_total_L[7] 
Empty$num_damaged7_2023[target_row]<-sub$number_damaged[7] 
Empty$total_damage7_2023[target_row]<-sub$total_damage[7]
Empty$avg_damage7_2023[target_row]<-sub$average_damage[7]
Empty$LAR_7_2023[target_row]<-sub$LAR[7] 


## Master notes
Empty$Master_notes_2023[target_row]<-sub$Master_notes[2] 
...
Empty$Mature_length_siliques_2023[target_row]<-sum(sub[,c(88,
                                                          90,
                                                          92,
                                                          94,
                                                          96,
...
check1 <-subset(Empty, PlantID=="Sch220")
check2 <-subset(Empty, PlantID=="M145")
...
write.xlsx(Empty, "Grasshopper2023_b.xlsx", sheetName="Summary2023", colnames=TRUE, rownames=TRUE, keepNA=TRUE) 
