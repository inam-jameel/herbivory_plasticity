...
##Data
gothic<-read.csv("inam_gothic_census23.csv", header=T)
sapply(gothic,class)
...
Empty_input<-read.csv("inam_gothic_empty23.csv", header=T)
...
ifelse((sub$Status[11]=="Alive"), Empty$Season_survival_2023[target_row]<-1, Empty$Season_survival_2023[target_row]<-0)
...
Empty$Exclude_2023[target_row]<-sub$Include[11] 
...
##I measured leaf damage at censuses 3, and 7

Empty$num_total3_2023[target_row]<-sub$Leaf_number[3] 
Empty$num_damaged3_2023[target_row]<-sub$Damaged_leaves[3] 
#Empty$total_damage3_2023[target_row]<-sub$total_damage[3]
#Empty$avg_damage3_2023[target_row]<-sub$average_damage[3]
Empty$LAR_3_2023[target_row]<-sub$LAR[3] 

Empty$num_total7_2023[target_row]<-sub$Leaf_number[7] 
Empty$num_damaged7_2023[target_row]<-sub$Damaged_leaves[7] 
...
write.xlsx(Empty, "gothic2023_summary.xlsx", sheetName="Summary2023", colnames=TRUE, rownames=TRUE, keepNA=TRUE) 
